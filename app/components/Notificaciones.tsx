    ...
    <div className="relative">
      ...
      {mostrar && (
        <>
          ...
          <div className="fixed z-50 overflow-hidden border shadow-xl left-64 top-20 w-80 bg-slate-800 border-slate-700 rounded-2xl">
                ...
                {notificaciones.map(n => (
                  <button
                    ...
                    className={`w-full text-left p-4 border-b border-slate-700/50 hover:bg-slate-700/50 transition-colors ${
                      n.tipo === 'peligro' ? 'border-l-2 border-l-red-500' : 'border-l-2 border-l-yellow-500'
                    }`}
                  >
                    <p className="text-sm font-medium text-white">{n.titulo}</p>
                    <p className="text-slate-400 text-xs mt-0.5">{n.mensaje}</p>
                    <p className={`text-xs mt-1 font-medium ${
                      n.tipo === 'peligro' ? 'text-red-400' : 'text-yellow-400'
                    }`}>
